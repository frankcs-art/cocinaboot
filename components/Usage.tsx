      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-1 bg-zinc-900 border border-white/5 p-6 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] h-fit">
          ...
          <div className="space-y-5 md:space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Tipo de Movimiento</label>
              <div className="flex gap-2 p-1 bg-white/5 rounded-xl border border-white/10">
                <button
                  onClick={() => setType('Consumo')}
                  className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${type === 'Consumo' ? 'bg-emerald-500 text-black' : 'text-slate-500 hover:text-white'}`}
                >
                  Consumo
                </button>
                <button
                  onClick={() => setType('Merma')}
                  className={`flex-1 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${type === 'Merma' ? 'bg-rose-500 text-black' : 'text-slate-500 hover:text-white'}`}
                >
                  Merma
                </button>
              </div>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Cantidad</label>
              <input 
                type="number" 
            ...
            <button 
              onClick={() => { if(selectedItem) { onRecordUsage(selectedItem, q, type); setSelectedItem(''); } }}
              className={`w-full py-4 md:py-5 rounded-xl md:rounded-3xl text-black font-black text-xs md:text-sm transition-all shadow-xl active:scale-95 ${type === 'Consumo' ? 'bg-emerald-500 hover:bg-emerald-400' : 'bg-rose-500 hover:bg-rose-400'}`}
            >
              Registrar {type}
            </button>
          </div>
               ...
               <table className="w-full text-left">
                  <thead className="bg-white/2">
                     <tr>
                        ...
                        <th className="px-6 md:px-10 py-5 text-[10px] font-black text-slate-600 uppercase tracking-widest">Tipo</th>
                     ...
                     {usage.map(u => (
                        <tr key={u.id} className="hover:bg-white/2 transition-colors">
                          ...
                          <td className="px-6 md:px-10 py-5 md:py-6">
                            <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${u.type === 'Consumo' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'}`}>
                              {u.type}
                            </span>
                          </td>
                          <td className="px-6 md:px-10 py-5 md:py-6 text-sm font-black text-white">{u.itemName}</td>
                          <td className={`px-6 md:px-10 py-5 md:py-6 text-right font-black ${u.type === 'Consumo' ? 'text-emerald-400' : 'text-rose-400'}`}>
                            -{u.quantityConsumed} {u.unit}
                          </td>
                        </tr>
                     ))}
                  </tbody>
                  ...
                  <div key={u.id} className="p-4 flex justify-between items-center bg-zinc-900/30">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[7px] font-black uppercase px-1.5 rounded-full ${u.type === 'Consumo' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'}`}>{u.type}</span>
                      </div>
                      <p className="text-[9px] text-slate-500 font-bold mt-1 uppercase">{new Date(u.date).toLocaleDateString()}</p>
                    </div>
                    <p className={`text-sm font-black ${u.type === 'Consumo' ? 'text-emerald-400' : 'text-rose-400'}`}>-{u.quantityConsumed} {u.unit}</p>
                  </div>
                ))}
             </div>
           </div>
